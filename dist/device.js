...
export class FastbootDevice {
    ...
    constructor(device, logger = window.console) {
        if (!device.serialNumber) {
            throw new Error("Access to USBDevice#serialNumber is necessary but stored only in temporary memory.");
        }
        this.device = device;
    ...
    setup() {
        if (this.device.configurations.length > 1) {
            console.warn(`device has ${this.device.configurations.length} configurations. Using the first one.`);
        }
    ...
    async sendCommand(text) {
        ...
        if (this.lastPacket &&
            ...
            return this.lastPacket; // TODO: Fix bad assertion
        }
    }
    async exec(command) {
        if (this.isActive) {
    ...
    async transferData(buffer) {
        ...
        if (xferHex.length !== 8) {
            throw new FastbootDeviceError("FAIL", `Transfer size overflow: ${xferHex} is more than 8 digits`);
        }
        this.logger.log(`Sending command download:${xferHex}.`);
        const response = await this.sendCommand(`download:${xferHex}`);
        if (response.status !== "DATA") {
            throw new FastbootDeviceError("FAIL", `response to download:${xferHex} is ${response.status}. Expected DATA.`);
        }
        else if (response.dataLength !== buffer.byteLength) {
            throw new FastbootDeviceError("FAIL", `Bootloader wants ${response.dataLength} bytes, requested to send ${buffer.byteLength} bytes`);
        }
        ...
        while (remainingBytes > 0) {
            ...
            if (i % 1000 === 0) {
                this.logger.log(`Sending ${chunk.byteLength} bytes to endpoint, ${remainingBytes} remaining, i=${i}`);
            }
