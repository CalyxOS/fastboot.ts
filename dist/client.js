...
export class FastbootClient {
    ...
    async flashing(command) {
        if ((command === "unlock" && (await this.unlocked())) ||
        ...
        for (const line of lines) {
            ...
            switch (command) {
                case "flash": {
                    ...
                    if (!entry) {
                        throw new Error(`partition ${partition} with filename ${filename} not found in zipfile.`);
                    }
                    ...
                    await this.doFlash(partition, // TODO: Assert this better
                    blob, slot, // TODO: Assert this better
                    applyVbmeta);
                    break;
                }
                case "reboot":
